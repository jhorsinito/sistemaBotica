(function(){
    angular.module('products.controllers',[])
        .controller('ProductController',['$scope', '$routeParams','$location','crudService' ,'$filter','$route','$log','ngProgressFactory','$rootScope','trouble','$window','$modal',
            function($scope, $routeParams,$location,crudService,$filter,$route,$log,ngProgressFactory,$rootScope,trouble,$window,$modal){
                ...
                ./ de variants create
                 */
                 $scope.validarTipoMedicamento = function($index){
                    if ($scope.product.tipoProducto_id==1) {
                        $scope.product.tipoMedicamento='Comercial';  
                    }else{
                        $scope.product.tipoMedicamento=null;
                        $scope.product.laboratorio_id=null;
                    }
                    $scope.grupoProductos=[];
                    $log.log($scope.product);
                };
                $scope.limpiarBuscarProducto = function($index){
                    $scope.productoSelected=undefined;
                };
                

                 $scope.productoSelected=undefined;
                $scope.getService = function(val) {
                    $scope.tipoMedicamentoBuscar="";
                    if ($scope.product.tipoMedicamento=='Comercial') {
                        $scope.tipoMedicamentoBuscar="Generico";
                    }else{
                        $scope.tipoMedicamentoBuscar='Comercial';
                    }
                  return crudService.recuperarDosDato('buscarProducto',$scope.tipoMedicamentoBuscar,val).then(function(response){
                ...
                $scope.grupoFarmaceuticoSelected=undefined;
                $scope.getServiceFarmaceutoco = function(val) {
                  return crudService.recuperarUnDato('buscarGrupoFarmaceutico',val).then(function(response){
                    return response.map(function(item){
                      return item;
                    });
                  });
                ...
                $scope.agregarGrupoFarmaceutico = function(val) {
                  if ($scope.grupoFarmaceuticoSelected.nombre==undefined) {
                        alert("Seleccione Correctamente un Grupo Farmaceutico");
                        $scope.grupoFarmaceuticoSelected=undefined;
                    }else{
                        $scope.grupoFarmaceutico.push($scope.grupoFarmaceuticoSelected);
                        $scope.grupoFarmaceuticoSelected=undefined;
                        $log.log($scope.grupoFarmaceutico);
                    }
                };
                $scope.eliminarGrupoFarmaceutico = function($index){
                    $scope.grupoFarmaceutico.splice($index,1);
                }


                $scope.agregarEditProducto = function(val) {
                    $scope.addDetComercialGenerico={};
                  if ($scope.productoSelected.nombre==undefined) {
                        alert("Seleccione Correctamente un Producto");
                        $scope.productoSelected=undefined;
                    }else{
                        if ($scope.product.tipoMedicamento=="Generico") {
                            $scope.addDetComercialGenerico.productoGenerico_id=$scope.product.id;
                            $scope.addDetComercialGenerico.productoComercial_id=$scope.productoSelected.id;
                        }else if ($scope.product.tipoMedicamento=="Comercial"){
                            $scope.addDetComercialGenerico.productoGenerico_id=$scope.productoSelected.id;
                            $scope.addDetComercialGenerico.productoComercial_id=$scope.product.id;
                        }
                        crudService.create( $scope.addDetComercialGenerico, 'detComercialGenericos').then(function (data) {
                                if ($scope.product.tipoMedicamento=="Generico") {
                                        crudService.byId($scope.product.id,"buscarGenerioco").then(function(data){
                                            $scope.grupoProductos = data;
                                        });

                                    }else if($scope.product.tipoMedicamento=="Comercial"){
                                        crudService.byId($scope.product.id,"buscarComercial").then(function(data){
                                            $scope.grupoProductos = data;
                                        });
                                    } 
                            });
                ...
                $scope.eliminarEditProducto = function(row){
                    crudService.destroy(row,'detComercialGenericos').then(function(data)
                    {
                        if ($scope.product.tipoMedicamento=="Generico") {
                            crudService.byId($scope.product.id,"buscarGenerioco").then(function(data){
                                $scope.grupoProductos = data;
                ...
                $scope.agregarEditGrupoFarmaceutico = function(val) {
                    $scope.addGrupoFarmaceuticoVariante={};
                  if ($scope.grupoFarmaceuticoSelected.nombre==undefined) {
                        alert("Seleccione Correctamente un Grupo Farmaceutico");
                        $scope.grupoFarmaceuticoSelected=undefined;
                    }else{
                        $scope.addGrupoFarmaceuticoVariante.grupoFarmacologico_id=$scope.grupoFarmaceuticoSelected.id;
                        $scope.addGrupoFarmaceuticoVariante.variant_id=$scope.variant.id;
                        
                        crudService.create( $scope.addGrupoFarmaceuticoVariante, 'detGrupoFarmaceuticos').then(function (data) {
                            crudService.recuperarUnDato('cargarDetGrupoFarmaceuticoVariantes',$scope.variant.id). then(function(data)
                            {
                                $scope.grupoFarmaceutico=data;
                            });
                        });
                ...
                $scope.eliminarEditGrupoFarmaceutico = function(row){
                    crudService.destroy(row,'detGrupoFarmaceuticos').then(function(data)
                    {
                        crudService.recuperarUnDato('cargarDetGrupoFarmaceuticoVariantes',$scope.variant.id). then(function(data){
                            $scope.grupoFarmaceutico=data;
                        });
                ...
                $scope.agregarProducto = function(val) {
                  if ($scope.productoSelected.nombre==undefined) {
                        alert("Seleccione Correctamente un Producto");
                        $scope.productoSelected=undefined;
                    }else{
                        $scope.grupoProductos.push($scope.productoSelected);
                        $scope.productoSelected=undefined;
                        $log.log($scope.grupoProductos);
                    }
                };
                $scope.eliminarProducto = function($index){
                    $scope.grupoProductos.splice($index,1);
                }
                ...
                if(id)
                {
                    if($location.path() == '/products/edit/'+$routeParams.id) {
                          ...
                          //  if(!$scope.product.hasVariants){
                                crudService.byforeingKey('variants','variant',id).then(function(data){
                                    ...
                                    if ($scope.product.tipoMedicamento=="Generico") {
                                        ...
                                        crudService.byId(id,"buscarComercial").then(function(data){
                                            $scope.grupoProductos = data;
                                        });
                                    }

                                    crudService.all('almacenes').then(function (data){
                        ...
                        //crudService.select('products', 'laboratorios').then(function (data) {
                          //  $scope.laboratorios = data;
                        //});

                        crudService.all('tipoProductos').then(function (data){
                    ...
                    if($location.path() == '/variants/edit/'+$routeParams.id) {
                        ...
                        crudService.recuperarUnDato('cargarDetGrupoFarmaceuticoVariantes',$routeParams.id). then(function(data)
                        {
                            $scope.grupoFarmaceutico=data;
                                $log.log('$scope.grupoFarmaceutico');
                                $log.log($scope.data);
                    ...
                    if($location.path() == '/products/create') {
                        ...
                        //crudService.all('laboratorios').then(function (data){
                                        //$scope.laboratorios = data;
                ...
                $scope.createProduct = function(){
                    ...
                    if ($scope.productCreateForm.$valid) {
                        ...
                        $scope.product.grupoProductos=$scope.grupoProductos;
                        $log.log($scope.product);
                ...
                $scope.createVariant = function(){
                    ...
                    $scope.variant.grupoFarmaceutico=$scope.grupoFarmaceutico;
   ...
   $scope.AddPres1 = function(){
               ...
               $scope.validacodigo=function(texto){
                 if(texto!=undefined){
                  crudService.validar('productsCodigo',texto).then(function (data){
                        if(data.codigo!=undefined){
                           alert("Usted no puede crear dos productos con el mismo codigo");
