...
use Salesfly\Salesfly\Entities\Producto;
...
class ProductoRepo extends BaseRepo{
    ...
    {
        //$promotion =Producto::select('id','nombre','codigo','estado')->where('nombre','like', $q.'%')
        //            //with(['customer','employee'])
        //            ->paginate(15);
        //return $promotion;
        $products = Producto::leftjoin('brands','products.brand_id','=','brands.id')
            ...
            ->select(\DB::raw('DISTINCT(products.id) as proId'),'products.codigo as proCodigo','products.nombreProducto as proNombre',
...
WHERE products.presentation_base = presentation.id and products.id = proId and products.hasVariants = false ) as detPresPri'))
            //->having()
            ->groupBy('products.id')
            ->where('products.nombreProducto','like',$q.'%')
            ->paginate(15);
        return $products;
    }
    public function searchProducts($q)
    {
        $products =Producto::select('id','nombre')
                    //with(['customer','employee'])
                    ->get();
        return $products;
    }
    ...
    {
        $products =Producto::select('id','nombre','codigo','estado','hasVariants')
                    //with(['customer','employee'])
                    ->where('nombre','like', $q.'%')
                    ->where('estado','=','1')
    ...
    public function paginate($qantity){
        ...
        //$products = Producto::find(1)->variant; trae 1
        //$products = Producto::find(1)->variants; trae todos
        //$products = $this->getModel()->with('brand','type','variant')->paginate($qantity);
        //$productsNovariants = Producto::join('brands','products.brand_id','=','brands.id')
       //                    ->join('types','products.type_id','=','types.id')
         //                   ->join('variants','products.id','=','variants.product_id')
        //                    ->where('products.hasVariants',false)
         //                   ->select('products.id as proId','products.codigo as proCodigo','products.nombreProducto as proNombre','variants.price as varPrice',
        ...
        //$productsSivariants =  Producto::join('brands','products.brand_id','=','brands.id')
          //                  ->join('types','products.type_id','=','types.id')
          //                  ->join('variants','products.id','=','variants.product_id')
          //                  ->where('products.hasVariants',true)
          //                  ->select(\DB::raw('DISTINCT(products.id) as proId'),'products.codigo as proCodigo','products.nombreProducto as proNombre','variants.price as varPrice',
          //                      'brands.nombre as braNombre','types.nombre as typNombre','products.created_at as proCreado',
...
WHERE products.presentation_base = presentation.id and products.id = proId and products.hasVariants = false ) as detPresPri'))
    ...
    public function Autocomplit(){
            $products = Producto::leftjoin('variants','products.id','=','variants.product_id')
                            ...
                            ->select(\DB::raw('products.id as proId,brands.nombre as BraName,types.nombre as TName,products.codigo as proCodigo,products.nombreProducto as proNombre,products.modelo as modelo,
    ...
    public function find($id){
        $oProduct = Producto::find($id);

        $producto = $oProduct->load(['station','type','brand','material']);
    ...
    public function pag()
    {
        ...
        /*$products = Producto::with(['variants' => function ($query) {
        ...
        */
        ...
        //$products = Producto::with(['brand'])->get();

        //return $products;
        /*$variants = $producto->variants->load(['detAtr' => function ($query) {
            $query->orderBy('atribute_id', 'asc');
        },'producto','detPre' => function($query) use ($producto){
            $query->join('presentation','presentation.id','=','detPres.presentation_id')
                ->where('presentation.id',$producto->presentation_base);
      ...
      $datos = \DB::table('products')->leftjoin('materials','products.material_id','=','materials.id')
                           ->leftjoin('variants','products.id','=','variants.product_id')
                            ...
                            ->select(\DB::raw('variants.sku as SKU ,detPres.id as detPre_id,variants.id as vari ,
                                ...
                                IF(products.hasVariants=1 , CONCAT(products.nombreProducto,"(",(SELECT GROUP_CONCAT(CONCAT(atributes.shortname,":",detAtr.descripcion) SEPARATOR " /")
                                 FROM variants
                                ...
                                GROUP BY variants.id),")"),  products.nombreProducto ) as NombreProducto,
                              ...
                              stock.stockPedidos as stockPedidos,stock.stockSeparados as stockSeparados,
                               IF(products.hasVariants=1 , CONCAT(variants.codigo," - ",products.nombreProducto,"/ ",(SELECT GROUP_CONCAT(CONCAT(atributes.shortname,":",detAtr.descripcion) SEPARATOR " /") FROM variants
                                LEFT JOIN detAtr ON detAtr.variant_id = variants.id
                                LEFT JOIN atributes ON atributes.id = detAtr.atribute_id
                                where variants.id=vari
                                GROUP BY variants.id)),  CONCAT(variants.codigo," - ",products.nombreProducto, IF(products.modelo is null ," ", CONCAT(" - ",products.modelo) ) ) ) as NombreAtributos , T1.nombre as Base, T2.nombre as Presentacion, products.presentation_base, warehouses.id as idAlmacen,
    ...
    public function misDatosVariantes($store,$were,$q){
      $datos = \DB::table('products')->leftjoin('materials','products.material_id','=','materials.id')
                           ->leftjoin('variants','products.id','=','variants.product_id')
                            ...
                            ->select(\DB::raw('variants.sku as SKU ,detPres.id as detPre_id,products.nombreProducto as NombreProducto,
                                materials.nombre as Material,
                              warehouses.nombre as Almacen,stock.stockActual as Stock,detPres.price as precioProducto,
                              ...
                              variants.id as vari , IF(products.hasVariants=1 , CONCAT(variants.codigo," - ",
                                products.nombreProducto,"/ ",(SELECT GROUP_CONCAT(CONCAT(atributes.shortname,":",detAtr.descripcion) 
                                    SEPARATOR " /") FROM variants
                                LEFT JOIN detAtr ON detAtr.variant_id = variants.id
                                LEFT JOIN atributes ON atributes.id = detAtr.atribute_id
                                where variants.id=vari
                                GROUP BY variants.id)),  CONCAT(variants.codigo," - ",products.nombreProducto, IF(products.modelo is null ,
    ...
    public function variantsAllInventary($store,$were,$q,$type,$brand,$producto){
      ...
      $datos = \DB::table('products')->leftjoin('materials','products.material_id','=','materials.id')
                           ->leftjoin('variants as T6','products.id','=','T6.product_id')
                            ...
                            ->select(\DB::raw('products.nombreProducto as Producto,products.modelo,T6.sku as codigo,T6.id as vari ,T7.stockActual as stock,T10.nombre as Linea,T12.nombre as Mate,
                                                ...
                                                IF(products.hasVariants=1 , CONCAT(T6.codigo," - ",products.nombreProducto,"/ ",(SELECT GROUP_CONCAT(CONCAT(atributes.shortname,":",detAtr.descripcion) SEPARATOR " /") FROM variants
                                                    LEFT JOIN detAtr ON detAtr.variant_id = variants.id
                                                    LEFT JOIN atributes ON atributes.id = detAtr.atribute_id
                                                    where variants.id=vari
                                                    GROUP BY variants.id)),  CONCAT(T6.codigo," - ",products.nombreProducto, IF(products.modelo is null ," ", CONCAT(" - ",products.modelo) ) ) ) as NombreAtributos,
                            ...
                            ->where('products.nombreProducto','like',$producto.'%')
                            //->orWhere('products.modelo','like',$producto.'%')
                            ->where('T9.id','like',$store.'%')
                            ->where('T8.id','like',$were.'%')
                            ->where('T10.id','like',$type.'%')
                            ->where('T12.id','like',$brand.'%')

                            ->orWhere('T6.codigo','like', $producto.'%')
                            ->where('T9.id','like',$store.'%')
                            ->where('T8.id','like',$were.'%')
                            ->where('T10.id','like',$type.'%')
                            ->where('T12.id','like',$brand.'%')
     ...
     public function validarNoRepitname($text){
        $products =producto::where('nombreProducto','=', $text)
                    //->with(['customer','employee'])
                    ->first();
        return $products;
    }
     public function TraerModelos(){
        $products =producto::select('id','modelo')
                    //->with(['customer','employee'])
                    ->where('modelo','<>','null')
                    ->groupBy('modelo')
                    ->get();
