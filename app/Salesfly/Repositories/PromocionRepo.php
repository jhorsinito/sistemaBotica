...
class PromocionRepo extends BaseRepo{
    ...
    public function listarPromociones(){
        $promocion=Promocion::join("variants","variants.id","=","promocion.productBase_id")
                              ...
                              ->select(\DB::raw("promocion.id,promocion.cantidad,promocion.descripcion,promocion.estado,CONCAT((SUBSTRING(promocion.fecha_inicio,9,2)),'-',(SUBSTRING(promocion.fecha_inicio,6,2)),'-',(SUBSTRING(promocion.fecha_inicio,1,4)))as fecha_inicio,CONCAT((SUBSTRING(promocion.fecha_fin,9,2)),'-',(SUBSTRING(promocion.fecha_fin,6,2)),'-',(SUBSTRING(promocion.fecha_fin,1,4)))as fecha_fin,promocion.descuento,products.nombre,variants.sku,variants.id as vari,promocion.product_id as vari2,
                    ...
                    (select CONCAT(pro.nombre,'/',vr.sku,'/',(select T20.descripcion FROM detAtr T20 where T20.variant_id=vari2 and T20.atribute_id=1),'/',(select T20.descripcion FROM detAtr T20 where T20.variant_id=vari2 and T20.atribute_id=2)) as descri FROM promocion pr inner join variants vr on pr.product_id=vr.id 
                        inner join products pro on pro.id=vr.product_id limit 1)as product2"))
