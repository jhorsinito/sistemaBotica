...
class ProductsController extends Controller
{
    ...
    \DB::beginTransaction();
        ...
        $grupoProductos=$request->grupoProductos;
        ...
        if($request->has('image') and substr($request->input('image'),5,5) === 'image'){
            ...
            //================================./PROD CON VARIANTES==============================//
            if ($request->input('tipoProducto_id')!=null) {
                
                
                if ($request->input('tipoMedicamento')=='Comercial') {
                    foreach($grupoProductos as $object){
                      $object['productoComercial_id'] = $temporal;
                      $object['productoGenerico_id']=$object['id'];
                      $object['id']=null;

                      $detComercialGenericoRepo;
                      $detComercialGenericoRepo = new DetComercialGenericoRepo;
                      $insertar=new DetComercialGenericoManager($detComercialGenericoRepo->getModel(),$object);
                      $insertar->save();
                      $detComercialGenericoRepo = null;           
                    }
                }else if ($request->input('tipoMedicamento')=='Generico') {
                    foreach($grupoProductos as $object){
                      $object['productoComercial_id'] = $object['id'];
                      $object['productoGenerico_id']=$temporal;
                      $object['id']=null;
   ...
   public function validarNombre($text){
    ...
    public function validarCodigo($text){
        $product = $this->productRepo->validarCodigo($text);
        return response()->json($product);
    }
    
    public function buscarProducto($dato1,$dato2)
    {
