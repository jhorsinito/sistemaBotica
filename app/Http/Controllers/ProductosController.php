...
use Salesfly\Salesfly\Repositories\ProductoRepo;
use Salesfly\Salesfly\Managers\ProductoManager;
...
class ProductosController extends Controller
{
    ...
    public function __construct(ProductoRepo $productoRepo, VariantRepo $variantRepo, DetPresRepo $detPres)
    {
    ...
    public function paginate(){
        $productos = $this->productoRepo->paginate(15);
        return response()->json($productos);
    }
    public function autocomplit(){
         $productos = $this->productoRepo->Autocomplit();
        return response()->json($productos);
    }
        public function pag(){
        $productos = $this->productoRepo->pag();
        return response()->json($productos);
        }
    public function misDatosVariantes($store,$were,$q){
        $productos = $this->productoRepo->misDatosVariantes($store,$were,$q);
        return response()->json($productos);
    }
    public function misDatos($store,$were,$q)
    {
        $productos = $this->productoRepo->misDatos($store, $were, $q);
        return response()->json($productos);
    }
    public function searchsku($store,$were,$q)
    {
        $productos = $this->productoRepo->searchsku($store, $were, $q);
        return response()->json($productos);
    }
    ...
    public function favoritos($store,$were,$q){
        $productos = $this->productoRepo->favoritos($store,$were,$q);
        return response()->json($productos);
    } 
    public function variantsAllInventary($store,$were,$q,$type,$marca,$product){
        //return $q;
        $productos = $this->productoRepo->variantsAllInventary($store,$were,$q,$type,$marca,$product);
        return response()->json($productos);
    } 
    public function TraerModelos(){
        $productos = $this->productoRepo->TraerModelos();
        return response()->json($productos);
    }
    public function form_create()
    {
        return View('productos.form_create');
    }
   
    public function form_edit()
    {
        return View('productos.form_edit');
    }
    ...
    \DB::beginTransaction();
        ...
        $product = $this->productoRepo->getModel();
        ...
        $managerPro = new ProductoManager($product,$request->except('sku','suppPri','markup','price','track'));
        //================================PROD CON VARIANTES==============================//
        ...
        if($request->has('image') and substr($request->input('image'),5,5) === 'image'){
            ...
            Image::make($image)->resize(200,200)->save('images/productos/'.$product->id.'.'.$mime);
            $product->image='/images/productos/'.$product->id.'.'.$mime;
            $product->save();
        }else{
            $product->image='/images/productos/product.png';
            $product->save();
        }
    ...
    {
        $product = $this->productoRepo->find($id);
        //sleep(5);
    ...
    public function edit(Request $request)
    {
        ...
        $product = $this->productoRepo->find($request->id);
    ...
    {
        //$customer= $this->productoRepo->find($request->id);
    ...
    {
        //$q = Input::get('q');
        $customers = $this->productoRepo->search($q);
   ...
   public function selectProducts(){
    ...
    {
        //$q = Input::get('q');
        $stations = $this->productoRepo->search($q);
    ...
    public function getAutocomplit2(){
        $product = $this->productoRepo->Autocomplit2();
        //sleep(5);
        return response()->json($product);
    }
    ...
    {
        //$q = Input::get('q');
        $productos = $this->productoRepo->searchProductAddVariant($q);

        return response()->json($productos);
    }
   
   public function validarNombre($text){
        $product = $this->productoRepo->validarNoRepitname($text);
        return response()->json($product);
    }
}
